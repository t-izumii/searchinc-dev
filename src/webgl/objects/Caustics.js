...
export class Caustic {
  constructor(size = 512, waterLevel = 0, waterSize = 4000) {
    this.size = size;
    this.waterLevel = waterLevel;
    this.waterSize = waterSize;

    // コースティクス用のRenderTarget（動的更新用の設定）
    this.causticsTarget = new THREE.WebGLRenderTarget(size, size, {
      minFilter: THREE.LinearFilter,
      magFilter: THREE.LinearFilter,
      format: THREE.RGBAFormat,
      generateMipmaps: false, // ミップマップを無効化（動的更新に必須）
  ...
  init() {
    ...
    this.material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uResolution: {
          // RenderTargetの解像度を指定（ShaderToy形式）
          value: new THREE.Vector3(this.size, this.size, 1.0),
        },
      },
      vertexShader: `
        varying vec2 vUv;
      ...
      fragmentShader: `
        ...
        float caustics(vec3 p, vec3 lp) {
            ...
            // スケール係数を大きくすると模様が小さくなる（例：0.1, 0.2, 0.5など）
            float scale = 2.0;
    ...
    // 正投影カメラ（真上から）
    const frustumSize = this.waterSize;
    ...
    this.camera.lookAt(0, this.waterLevel, 0);
  }

  update(renderer, time) {
    // デバッグ：timeが更新されているか確認
    if (Math.floor(time) % 2 === 0 && time % 1 < 0.01) {
    }

    // uniformを更新
    this.material.uniforms.uTime.value = time * 0.03;

    // コースティクスをRenderTargetにレンダリング
    renderer.setRenderTarget(this.causticsTarget);
    renderer.render(this.scene, this.camera);
    renderer.setRenderTarget(null);

    // テクスチャのバージョンを強制的にインクリメント
    this.causticsTarget.texture.version++;
  }
